...
import kotlinx.android.synthetic.main.holder_favoritos.view.*

class FavoritosViewHolder(view: View, val context: Context) : RecyclerView.ViewHolder(view), View.OnClickListener {

    val localPicture = view.localPicture
    val localName = view.localName
    val localTipo = view.localTipo
    val localNota = view.localNota
    val localFavorito = view.localFavorito
    val localUltimaAtualizacao = view.localUltimaAtualizacao
    ...
    fun bind(local: ViewType) {
        ...
        if (local is Estabelecimento) {
            ...
            localUltimaAtualizacao.text = "Atualizado por ${local.usuario.nome} em ${local.dataAtualizacao.displayAsDate()}"
