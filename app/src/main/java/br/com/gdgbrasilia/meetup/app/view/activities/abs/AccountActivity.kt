package br.com.gdgbrasilia.meetup.app.view.activities.abs
...
import br.com.gdgbrasilia.meetup.app.view.activities.TermosActivity
...
abstract class AccountActivity : AppCompatActivity(), FacebookCallback<LoginResult> {
    ...
    override fun onSuccess(result: LoginResult?) {
        accountViewModel.facebookLogin(result?.accessToken?.token!!) { usuario, mensagem ->
            if (usuario != null) {
                AppStatics.currentUser = usuario
                val bundle = Bundle()
                bundle.putBoolean(IntentFieldNames.IS_INITIAL_FLOW, true)
                startActivity(TermosActivity::class.java, bundle)
